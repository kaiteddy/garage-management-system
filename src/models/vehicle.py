    ...
    def fetch_dvla_data(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.mot_expiry = datetime.strptime(vehicle_data.get(
                    'motExpiryDate'), '%Y-%m-%d').date() if vehicle_data.get('motExpiryDate') else None
                self.tax_due = datetime.strptime(vehicle_data.get(
                    'taxDueDate'), '%Y-%m-%d').date() if vehicle_data.get('taxDueDate') else None
