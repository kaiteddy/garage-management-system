        ...
        with self.get_connection() as conn:
            ...
            for customer in data:
                try:
                    ...
                    print(
                        f"✅ Imported customer: {customer_data['name']} (ID: {customer_id})")

                except Exception as e:
                    print(
                        f"❌ Error importing customer {customer.get('name', 'Unknown')}: {e}")
                ...
                try:
                    ...
                    if customer_id:
                        ...
                        if not cursor.fetchone():
                            print(
                                f"⚠️ Skipping vehicle {vehicle['registration']}: Customer ID {customer_id} not found")
                            continue

                    # Prepare data with defaults
                    vehicle_data = {
                        # Normalize registration
                        'registration': vehicle['registration'].upper().replace(' ', ''),
                    ...
                    print(
                        f"✅ Imported vehicle: {vehicle_data['registration']} {vehicle_data['make']} {vehicle_data['model']} (ID: {vehicle_id})")

                except sqlite3.IntegrityError as e:
                    if "UNIQUE constraint failed" in str(e):
                        print(
                            f"⚠️ Vehicle {vehicle['registration']} already exists")
                    else:
                        print(
                            f"❌ Database error for vehicle {vehicle.get('registration', 'Unknown')}: {e}")
                except Exception as e:
                    print(
                        f"❌ Error importing vehicle {vehicle.get('registration', 'Unknown')}: {e}")
        ...
        with self.get_connection() as conn:
            ...
            for tech in data:
                try:
                    ...
                    print(
                        f"✅ Imported technician: {tech_data['first_name']} {tech_data['last_name']} (ID: {tech_id})")

                except Exception as e:
                    print(
                        f"❌ Error importing technician {tech.get('first_name', 'Unknown')}: {e}")
...
if __name__ == "__main__":
    ...
    customer_ids = importer.import_from_json(
        'sample_customers.json', 'customers')
    vehicle_ids = importer.import_from_json('sample_vehicles.json', 'vehicles')
    technician_ids = importer.import_from_json(
