...
def check_google_drive_sync():
    ...
    if status.get('connected', False):
        ...
        print(f"  - Auto-sync enabled: {config.get('auto_sync_enabled', False)}")
        print(f"  - Sync interval: {config.get('sync_interval_minutes', 0)} minutes")
        ...
        if last_sync:
            try:
                ...
                print(f"  - Last sync: {last_sync} ({diff.total_seconds() / 60:.1f} minutes ago)")
            except:
        ...
        for data_type, mapping in folder_mappings.items():
            print(f"  - {data_type}: {mapping.get('folder_name', 'Unknown')} (ID: {mapping.get('folder_id', 'Unknown')})")
            print(f"    Last modified: {mapping.get('last_modified', 'Never')}")
...
def check_data_linking():
    ...
    print(f"  - Customer linking rate: {stats.get('customer_linking_rate', 0):.2f}")
    ...
    print(f"  - Vehicle linking rate: {stats.get('vehicle_linking_rate', 0):.2f}")
