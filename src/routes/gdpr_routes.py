...
def delete_customer_data(customer_id):
    ...
    try:
        ...
        deletion_reason = data.get('reason', 'Customer request for data deletion')
        ...
        deletion_result = gdpr_service.delete_customer_data(customer_id, deletion_reason)
...
def get_compliance_status():
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM consent_records WHERE granted = 1')
        active_consents = cursor.fetchone()[0] if 'consent_records' in gdpr_tables else 0
        
        # Get access log statistics
        cursor.execute('SELECT COUNT(*) FROM data_access_log WHERE access_date > date("now", "-30 days")')
        recent_access = cursor.fetchone()[0] if 'data_access_log' in gdpr_tables else 0
        ...
        return jsonify({
            ...
            'compliance_status': {
                ...
                'recommendations': [
                    'Implement consent collection in customer registration' if not compliance_checks['consent_management_active'] else None,
                    'Enable access logging for all customer data operations' if not compliance_checks['access_logging_active'] else None
                ]
            }
        })
