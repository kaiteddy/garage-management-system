...
def get_job_sheet_templates():
    ...
    try:
        ...
        templates = job_sheets_service.get_job_sheet_templates(
            service_type, active_only)
...
def add_digital_signature(job_sheet_id):
    ...
    try:
        ...
        required_fields = ['signature_type',
                           'signature_data', 'signer_name', 'signer_role']
        for field in required_fields:
            if not data.get(field):
                return jsonify({
                    'success': False,
...
def get_job_sheet_statistics():
    ...
    try:
        ...
        completion_rate = (completed_sheets / total_sheets *
                           100) if total_sheets > 0 else 0
        ...
        tech_signature_rate = (
            signature_stats[1] / total_sheets * 100) if total_sheets > 0 else 0
        customer_signature_rate = (
            signature_stats[2] / total_sheets * 100) if total_sheets > 0 else 0
