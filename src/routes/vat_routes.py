...
def generate_vat_return():
    ...
    try:
        ...
        vat_return_data = vat_service.generate_vat_return_data(period_start, period_end)
    ...
    try:
        ...
        return jsonify({
            ...
            'compliance_status': {
                ...
                'recommendations': [
                    'Configure VAT registration number' if not compliance_checks['vat_registered'] else None,
                    'Complete business details' if not compliance_checks['business_details_complete'] else None,
                    'Set accounting periods' if not compliance_checks['accounting_period_set'] else None,
                    'Configure HMRC API credentials' if not compliance_checks['hmrc_credentials_configured'] else None
                ]
            }
        })
