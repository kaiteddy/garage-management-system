...
class DeepSourceFixManager:
    ...
    def __init__(self, project_root: str = None):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(self.project_root /
                                    'logs' / 'fix_manager.log'),
    ...
    def record_fix(self, fix_record: FixRecord) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                self.logger.info(
                    f"Recorded fix: {fix_record.fix_id} - {fix_record.fix_description}")
                return True
    ...
    def get_fix_statistics(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Success rate
            cursor = conn.execute(
                'SELECT COUNT(*) FROM fixes WHERE success = 1')
            successful_fixes = cursor.fetchone()[0]
            stats['success_rate'] = (
                successful_fixes / stats['total_fixes'] * 100) if stats['total_fixes'] > 0 else 0
    ...
    def should_auto_fix(self, issue_type: str, file_path: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if daily_count >= max_daily:
                self.logger.warning(
                    f"Daily auto-fix limit reached: {daily_count}/{max_daily}")
                return False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stats', action='store_true',
                        help='Show fix statistics')
    parser.add_argument('--history', action='store_true',
                        help='Show recent fix history')
