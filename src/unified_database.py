...
class UnifiedDatabase:
    ...
    def __init__(self, db_path: str = None):
        ...
        self.db_path = db_path or os.path.join(os.path.dirname(__file__), 'garage_management.db')
    ...
    def initialize_unified_schema(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_mot_records_vehicle_id ON mot_records (vehicle_id)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_mot_records_expiry_date ON mot_records (expiry_date)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_job_sheets_customer_id ON job_sheets (customer_id)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_job_sheets_vehicle_id ON job_sheets (vehicle_id)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_appointments_date ON appointments (appointment_date)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_sms_customer_id ON sms_communications (customer_id)')
                
                conn.commit()
                self.logger.info("✅ Unified database schema created successfully")
                
        except Exception as e:
            self.logger.error(f"❌ Failed to create unified database schema: {e}")
            raise
    ...
    def migrate_existing_data(self):
        ...
        try:
            ...
            mot_db_path = os.path.join(os.path.dirname(__file__), 'garage_management.db')
    ...
    def _migrate_mot_data(self, source_db_path: str):
        ...
        try:
            ...
            if source_cursor.fetchone():
                ...
                target_conn.commit()
                self.logger.info(f"✅ Migrated {len(mot_vehicles)} MOT vehicles")
