            ...
            if google_drive_service.is_available():
                ...
                if config.get('auto_sync_enabled', False):
                    ...
                    if result.get('success', False):
                        print(
                            f"✅ Successfully synced data from Google Drive: {result.get('total_imported', 0)} items imported")
                    else:
                        print(
                            f"⚠️ Sync from Google Drive completed with issues: {result.get('error', 'Unknown error')}")
                else:
