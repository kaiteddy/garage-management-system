...
class IntegratedUploadSystem {
    ...
    async handleUpload() {
        ...
        try {
            ...
            const response = await this.uploadWithProgress('/api/csv', formData, (progress) => {
                if (progressBar) {
...
window.confirmBulkOperation = function(operation) {
    if (confirm(`Are you sure you want to ${operation} all data? This action cannot be undone.`)) {
