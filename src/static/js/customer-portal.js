...
class CustomerPortal {
    ...
    async loadCustomerData() {
        ...
        try {
            ...
            // Load vehicles
            const vehiclesResponse = await fetch(`/api/vehicles?customer_id=${customerId}`);
    ...
    getLastServiceDate(vehicleId) {
        const vehicleServices = this.serviceHistory.filter(job => job.vehicle_id === vehicleId);
        if (vehicleServices.length === 0) return 'No services';
        
        const lastService = vehicleServices.sort((a, b) => new Date(b.created_date) - new Date(a.created_date))[0];
    ...
    renderServiceHistory() {
        ...
        const sortedHistory = this.serviceHistory.sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
        
        timeline.innerHTML = sortedHistory.map(job => `
    ...
    populateVehicleFilters() {
        ...
        if (yearFilter && this.serviceHistory.length > 0) {
            const years = [...new Set(this.serviceHistory.map(job => new Date(job.created_date).getFullYear()))];
            years.sort((a, b) => b - a);
    ...
    async loadAppointments(customerId) {
        try {
            const response = await fetch(`/api/appointments?customer_id=${customerId}`);
            if (response.ok) {
                const result = await response.json();
                const appointments = result.appointments || [];
                
                const today = new Date();
                const upcoming = appointments.filter(apt => new Date(apt.appointment_date) >= today);
                const past = appointments.filter(apt => new Date(apt.appointment_date) < today);
    ...
    filterServiceHistory() {
        ...
        if (vehicleId) {
            filteredHistory = filteredHistory.filter(job => job.vehicle_id == vehicleId);
        }
        
        if (year) {
            filteredHistory = filteredHistory.filter(job => new Date(job.created_date).getFullYear() == year);
        }
        ...
        const sortedHistory = filteredHistory.sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
    ...
    openBookingWidget(vehicleId = null) {
        ...
        alert('Booking widget would open here. In a real implementation, this would redirect to the online booking system.');
    }
