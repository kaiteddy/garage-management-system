...
class JobSheetsManager {
    ...
    renderJobSheetsList() {
        ...
        grid.innerHTML = this.jobSheets.map(sheet => this.createJobSheetCardHTML(sheet)).join('');
    }
    ...
    loadTemplate() {
        ...
        if (template) {
            document.getElementById('work-instructions').value = template.default_instructions || '';
            document.getElementById('safety-notes').value = template.default_safety_notes || '';
            document.getElementById('parts-required').value = template.default_parts || '[]';
            document.getElementById('tools-required').value = template.default_tools || '[]';
            document.getElementById('quality-checks').value = template.default_checks || '[]';
        }
    }
    ...
    searchJobSheets() {
        const searchTerm = document.getElementById('job-sheet-search').value.toLowerCase();
...
class QuotesManager {
    ...
    renderQuotesList() {
        ...
        grid.innerHTML = this.quotes.map(quote => this.createQuoteCardHTML(quote)).join('');
    }
    ...
    loadCustomerVehicles() {
        ...
        const customerVehicles = this.vehicles.filter(v => v.customer_id == customerId);
    ...
    calculateTotal() {
        const labourCost = parseFloat(document.getElementById('quote-labour-cost').value) || 0;
        const partsCost = parseFloat(document.getElementById('quote-parts-cost').value) || 0;
    ...
    async createQuote() {
        try {
            const formData = {
                ...
                vehicle_id: document.getElementById('quote-vehicle-select').value || null,
    ...
    async convertToJob(quoteId) {
        try {
            ...
            if (result.success) {
                this.showNotification(`Quote converted to job ${result.job_number}`, 'success');
                this.loadQuotes();
            } else {
            ...
            <div class="quote-detail">
                ...
                <div class="quote-breakdown">
                    ...
                    <div class="cost-table">
                        ...
                        <div class="cost-row">
                            <span>VAT (20%):</span>
                            <span>£${(((quote.labour_cost || 0) + (quote.parts_cost || 0)) * 0.20).toFixed(2)}</span>
                        </div>
