...
class MOTDashboard {
    ...
    bindModalEvents() {
        ...
        document.querySelector('#vehicleDetailsModal .modal-close')?.addEventListener('click', () => {
            this.hideModal('vehicleDetailsModal');
        });

        // Upload Modal
        document.querySelector('#uploadVehiclesModal .modal-close')?.addEventListener('click', () => {
    ...
    updateStatistics() {
        ...
        const critical = this.vehicles.filter(v => !v.is_expired && v.days_until_expiry <= 7);
        const dueSoon = this.vehicles.filter(v => !v.is_expired && v.days_until_expiry > 7 && v.days_until_expiry <= 30);
        const valid = this.vehicles.filter(v => !v.is_expired && v.days_until_expiry > 30);
        ...
        updateElement('sms-with-mobile', this.vehicles.filter(v => v.mobile_number).length);
        updateElement('sms-urgent', this.vehicles.filter(v => (v.is_expired || v.days_until_expiry <= 7) && v.mobile_number).length);

        // Calculate and update SMS coverage
        const withMobile = this.vehicles.filter(v => v.mobile_number).length;
        const coverage = this.vehicles.length > 0 ? Math.round((withMobile / this.vehicles.length) * 100) : 0;
    ...
    applyFilters() {
        ...
        switch (this.currentFilter) {
            case 'expired':
                ...
                break;
            case 'critical':
                filtered = filtered.filter(v => !v.is_expired && v.days_until_expiry <= 7);
                break;
            case 'due_soon':
                filtered = filtered.filter(v => !v.is_expired && v.days_until_expiry > 7 && v.days_until_expiry <= 30);
                break;
            case 'valid':
                filtered = filtered.filter(v => !v.is_expired && v.days_until_expiry > 30);
                break;
            case 'with_mobile':
        ...
        if (this.searchTerm) {
            filtered = filtered.filter(v => 
                ...
                (v.customer_name && v.customer_name.toLowerCase().includes(this.searchTerm)) ||
    ...
    async refreshVehicleData(registration) {
        try {
            const response = await fetch(`/api/mot/vehicles/${registration}/refresh`, {
                method: 'POST'
            });
    ...
    exportData() {
        ...
        if (link.download !== undefined) {
            ...
            link.setAttribute('download', `mot_vehicles_${new Date().toISOString().split('T')[0]}.csv`);
     ...
     */
    generateCSV() {
        const headers = ['Registration', 'Make', 'Model', 'Customer Name', 'Mobile', 'Email', 'MOT Expiry', 'Days Until Expiry', 'Status'];
