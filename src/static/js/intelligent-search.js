...
class IntelligentSearch {
    ...
    setupSearchInputs() {
        ...
        const searchInputs = document.querySelectorAll('.intelligent-search, [data-intelligent-search="true"]');
    ...
    createSuggestionContainer(input) {
        ...
        if (container && container.classList.contains('intelligent-search-suggestions')) {
            return container;
        }
    ...
    getSearchType(input) {
        ...
        if (input.id.includes('customer') || input.closest('.customer-search')) {
            return 'customers';
        } else if (input.id.includes('vehicle') || input.closest('.vehicle-search')) {
            return 'vehicles';
    ...
    async performSearch(query, searchType, suggestionContainer, inputElement) {
        try {
            ...
            if (this.searchCache.has(cacheKey)) {
                ...
                this.displaySuggestions(cachedResults, suggestionContainer, inputElement);
                return;
            }
    ...
    displaySuggestions(data, container, inputElement) {
        ...
        if (data.results) {
            ...
            if (data.results.customers) {
                suggestions = suggestions.concat(data.results.customers.slice(0, 3).map(item => ({
                    ...item,
                    type: 'customer',
                    display: item.name,
            ...
            if (data.results.vehicles) {
                suggestions = suggestions.concat(data.results.vehicles.slice(0, 3).map(item => ({
                    ...item,
                    type: 'vehicle',
            ...
            if (data.results.jobs) {
                suggestions = suggestions.concat(data.results.jobs.slice(0, 2).map(item => ({
                    ...item,
                    type: 'job',
    ...
    autoFillRelatedFields(suggestion, inputElement) {
        const form = inputElement.closest('form') || inputElement.closest('.form-container');
        if (!form) return;
    ...
    handleKeyboardNavigation(event, container) {
        ...
        const currentActive = container.querySelector('.search-suggestion-item.active');
    ...
    setupCustomerSearch() {
        ...
        const customerSearchInputs = document.querySelectorAll('#customer-search, .customer-search-input');
    ...
    setupVehicleSearch() {
        ...
        const vehicleSearchInputs = document.querySelectorAll('#vehicle-search, .vehicle-search-input');
    ...
    setupJobSearch() {
        ...
        const jobSearchInputs = document.querySelectorAll('#job-search, .job-search-input');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const needsIntelligentSearch = document.querySelectorAll('.intelligent-search, [data-intelligent-search="true"]').length > 0;
    ...
    if (needsIntelligentSearch) {
        ...
        console.log('🔍 No intelligent search inputs found, skipping initialization');
    }
});
