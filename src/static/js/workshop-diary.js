...
class WorkshopDiary {
    ...
    renderResources() {
        ...
        if (techList && this.technicians.length > 0) {
            techList.innerHTML = this.technicians.map(tech => `
        ...
        if (baysList && this.bays.length > 0) {
            baysList.innerHTML = this.bays.map(bay => `
    ...
    renderDayAppointments(date) {
        ...
        return dayAppointments.map(apt => this.createAppointmentHTML(apt)).join('');
    }
    ...
    renderMonthDayAppointments(date) {
        ...
        return dayAppointments.slice(0, 3).map(apt => `
            <div class="month-appointment ${apt.priority.toLowerCase()}" title="${apt.description}">
                ${apt.start_time} - ${apt.customer_name}
            </div>
        `).join('') + (dayAppointments.length > 3 ? `<div class="more-appointments">+${dayAppointments.length - 3} more</div>` : '');
    }
    ...
    updateDisplay() {
        document.getElementById('current-date-display').textContent = this.formatDateDisplay();
        this.renderCalendar();
        this.loadAppointments();
    }
    ...
    handleDrop(event) {
        try {
            ...
            try {
                appointmentId = event.dataTransfer ? event.dataTransfer.getData('text/plain') : null;
            } catch (transferError) {
            ...
            if (appointmentId && newDate) {
                ...
                console.warn('⚠️ Missing drop data - ID:', appointmentId, 'Date:', newDate);
            }
        } catch (error) {
    ...
    async moveAppointment(appointmentId, newDate) {
        try {
            ...
            const appointment = this.appointments ? this.appointments.find(apt => apt && apt.id === appointmentId) : null;
            if (!appointment) {
            ...
            setTimeout(() => {
                if (this.loadAppointments && typeof this.loadAppointments === 'function') {
                    this.loadAppointments();
                }
            }, 1000);
