...
class DataLoader:
    ...
    def get_vehicles(self, customer_id: Optional[int] = None, include_archived: bool = True) -> List[Dict]:
        ...
        if customer_id:
            vehicles = [v for v in vehicles if v.get("customer_id") == customer_id]
    ...
    def get_jobs(self, customer_id: Optional[int] = None, limit: Optional[int] = None) -> List[Dict]:
        ...
        jobs = sorted(jobs, key=lambda x: x.get("created_date", ""), reverse=True)
    ...
    def get_stats(self) -> Dict:
        ...
        active_jobs = [j for j in jobs if j.get("status", "").upper() != "COMPLETED"]
        
        # Calculate pending invoices
        pending_invoices = [i for i in invoices if i.get("status", "").upper() == "PENDING"]
        
        # Calculate monthly revenue (sum of completed jobs this month)
        completed_jobs = [j for j in jobs if j.get("status", "").upper() == "COMPLETED"]
        monthly_revenue = sum(j.get("total_amount", 0) for j in completed_jobs[:100])  # Sample
        
        # Calculate vehicles due MOT (sample calculation)
        vehicles_due_mot = len([v for v in vehicles[:100] if v.get("mot_expiry")])
    ...
    def get_kanban_jobs(self) -> Dict:
        ...
        for job in sample_jobs:
            ...
            kanban_job = {
                ...
                "description": job.get("description", "")[:100],  # Truncate for display
    ...
    def get_quotes(self) -> List[Dict]:
        ...
        for i, job in enumerate(sample_jobs):
            quote = {
                ...
                "customer_name": job.get("customer_name", "Unknown Customer"),
                "amount": job.get("total_amount", 0) * 1.1,  # Slightly higher than job
    ...
    def get_technicians(self) -> List[Dict]:
        ...
        return [
            {"id": 1, "name": "Tom Anderson", "specialization": "Engine Specialist", "available": True},
            {"id": 2, "name": "Lisa Brown", "specialization": "Electrical Systems", "available": True},
            {"id": 3, "name": "Mike Wilson", "specialization": "Brake Systems", "available": False},
            {"id": 4, "name": "Sarah Davis", "specialization": "MOT Testing", "available": True}
        ]
    ...
    def get_job_sheet_templates(self) -> List[Dict]:
        ...
        return [
            {"id": 1, "name": "Standard Service", "description": "Basic service template"},
            {"id": 2, "name": "MOT Test", "description": "MOT test template"},
            {"id": 3, "name": "Brake Service", "description": "Brake system service template"},
            {"id": 4, "name": "Engine Diagnostic", "description": "Engine diagnostic template"}
        ]
