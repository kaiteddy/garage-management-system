...
# MOT API endpoints (for frontend compatibility)
@app.route('/api/mot/vehicles')
def get_mot_vehicles():
    """Get MOT vehicles - proxy to MOT blueprint"""
    try:
        # Import MOT service to get actual data
        ...
        print(f"Error in get_mot_vehicles: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/mot/failed-registrations')
def get_mot_failed_registrations():
    """Get MOT failed registrations - proxy to MOT blueprint"""
    try:
        # Import MOT service to get actual data
        from mot_service import IntegratedMOTService
        mot_service = IntegratedMOTService()
        ...
        failed_registrations = mot_service.get_failed_registrations(status=status)

        return jsonify({
            'success': True,
            'failed_registrations': failed_registrations,
            'count': len(failed_registrations)
        })
    except Exception as e:
        print(f"Error in get_mot_failed_registrations: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/mot/sms/status')
def get_mot_sms_status():
    """Get SMS service status - proxy to MOT blueprint"""
    try:
        # Import MOT service to get actual data
        from mot_service import IntegratedMOTService
        mot_service = IntegratedMOTService()
        ...
        print(f"Error in get_mot_sms_status: {e}")
        return jsonify({'error': str(e)}), 500
...
if __name__ == '__main__':
    ...
    print("📊 Main interface: http://127.0.0.1:5002")
    print("🚗 MOT Reminders: http://127.0.0.1:5002/mot")
    print("📤 Upload interface: http://127.0.0.1:5002/upload")
