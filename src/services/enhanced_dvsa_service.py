...
class EnhancedDVSAService:
    ...
    def __init__(self, db_path: str):
        ...
        self.dvsa_client_id = os.getenv('DVSA_CLIENT_ID', '2b3911f4-55f5-4a86-a9f0-0fc02c2bff0f')
    ...
    def get_dvsa_access_token(self) -> Optional[str]:
        ...
        try:
            ...
            response = requests.post(self.dvsa_token_url, data=payload, timeout=30)
            ...
            if response.status_code == 200:
                ...
                print(f"Error getting DVSA access token: {response.status_code} - {response.text}")
                return None
    ...
    def get_mot_data(self, registration: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"DVSA API error: {response.status_code} - {response.text}")
                return None
    ...
    def get_tax_data(self, registration: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = requests.post(self.dvla_base_url, headers=headers, json=data, timeout=30)
    ...
    def get_comprehensive_vehicle_data(self, registration: str) -> Dict:
        ...
        try:
            ...
            if vehicle_info.get('mot_expiry'):
                try:
                    expiry_date = datetime.strptime(vehicle_info['mot_expiry'], '%Y-%m-%d')
            ...
            if vehicle_info.get('tax_due'):
                try:
                    tax_date = datetime.strptime(vehicle_info['tax_due'], '%Y-%m-%d')
    ...
    def schedule_automated_reminders(self, registration: str, customer_id: int = None) -> Dict:
        ...
        try:
            ...
            email = None
            if customer_id:
                cursor.execute('SELECT mobile, email FROM customers WHERE id = ?', (customer_id,))
            ...
            if vehicle_data.get('mot_expiry'):
                try:
                    mot_expiry = datetime.strptime(vehicle_data['mot_expiry'], '%Y-%m-%d')
            ...
            if vehicle_data.get('tax_due'):
                try:
                    tax_due = datetime.strptime(vehicle_data['tax_due'], '%Y-%m-%d')
