...
class IntelligentSearchEngine:
    ...
    def _score_customer_match(self, customer: Dict[str, Any], query: str) -> float:
        ...
        if customer.get('mobile'):
            ...
            if mobile_norm and query_phone:
                similarity = self.calculate_similarity(
                    mobile_norm, query_phone, fuzzy=False)
                if similarity > 0.8:  # High threshold for phone numbers
                    max_score = max(max_score, similarity *
                                    self.search_weights['mobile_phone'])
        ...
        if customer.get('phone'):
            ...
            if phone_norm and query_phone:
                similarity = self.calculate_similarity(
                    phone_norm, query_phone, fuzzy=False)
                if similarity > 0.8:
                    max_score = max(max_score, similarity *
                                    self.search_weights['mobile_phone'] * 0.9)

        # Check email
        if customer.get('email'):
            email_similarity = self.calculate_similarity(
                customer['email'], query)
            max_score = max(max_score, email_similarity *
                            self.search_weights['email'])

        # Check postal code
        if customer.get('postcode'):
            postal_norm = self.normalize_postal_code(customer['postcode'])
            if postal_norm and query_postal:
                postal_similarity = self.calculate_similarity(
                    postal_norm, query_postal, fuzzy=False)
                max_score = max(max_score, postal_similarity *
                                self.search_weights['postal_code'])

        # Check name (split into words for better matching)
        if customer.get('name'):
            name_similarity = self.calculate_similarity(
                customer['name'], query)
            max_score = max(max_score, name_similarity *
                            self.search_weights['name'])

        # Check account number
        if customer.get('account_number'):
            account_similarity = self.calculate_similarity(
                str(customer['account_number']), query, fuzzy=False)
            max_score = max(max_score, account_similarity *
                            self.search_weights['account_number'])
    ...
    def _score_vehicle_match(self, vehicle: Dict[str, Any], query: str) -> float:
        ...
        if vehicle.get('registration'):
            ...
            if reg_norm and query_reg:
                reg_similarity = self.calculate_similarity(
                    reg_norm, query_reg, fuzzy=False)
                if reg_similarity > 0.8:  # High threshold for registration
                    max_score = max(max_score, reg_similarity *
                                    self.search_weights['registration'])
        ...
        if vehicle.get('model'):
            model_similarity = self.calculate_similarity(
                vehicle['model'], query)
        ...
        if vehicle.get('customer_mobile'):
            ...
            if mobile_norm and query_phone:
                phone_similarity = self.calculate_similarity(
                    mobile_norm, query_phone, fuzzy=False)
    ...
    def _score_job_match(self, job: Dict[str, Any], query: str) -> float:
        ...
        if job.get('job_number'):
            job_num_similarity = self.calculate_similarity(
                str(job['job_number']), query, fuzzy=False)
    ...
    def find_customer_matches(self, customer_data: Dict[str, Any], threshold: float = 0.8) -> List[Tuple[int, float]]:
        ...
        for row in cursor.fetchall():
            ...
            score = self._calculate_customer_link_score(
    ...
    def find_vehicle_matches(self, vehicle_data: Dict[str, Any], threshold: float = 0.9) -> List[Tuple[int, float]]:
        ...
        for row in cursor.fetchall():
            ...
            score = self._calculate_vehicle_link_score(
                vehicle_data, existing_vehicle)
            if score >= threshold:
    ...
    def _calculate_customer_link_score(self, new_data: Dict[str, Any], existing_data: Dict[str, Any]) -> float:
        ...
        if new_data.get('postcode') and existing_data.get('postcode'):
            ...
            if postal1 and postal2:
                postal_similarity = self.calculate_similarity(
                    postal1, postal2, fuzzy=False)
                if postal_similarity > 0.9:
                    scores.append(postal_similarity *
                                  self.search_weights['postal_code'])
    ...
    def _calculate_vehicle_link_score(self, new_data: Dict[str, Any], existing_data: Dict[str, Any]) -> float:
        ...
        if new_data.get('registration') and existing_data.get('registration'):
            ...
            if reg1 and reg2:
                ...
                if similarity > 0.95:  # Very high threshold for registration
                    scores.append(
                        similarity * self.search_weights['registration'])
        ...
        if (new_data.get('make') and existing_data.get('make') and
            ...
            make_sim = self.calculate_similarity(
                new_data['make'], existing_data['make'])
            model_sim = self.calculate_similarity(
                new_data['model'], existing_data['model'])
