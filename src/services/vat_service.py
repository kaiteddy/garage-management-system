...
class VATService:
    ...
    def __init__(self, db_path: str, hmrc_client_id: str = None, hmrc_client_secret: str = None):
        ...
        self.hmrc_client_secret = hmrc_client_secret or os.getenv(
            'HMRC_CLIENT_SECRET')
    ...
    def calculate_vat(self, net_amount: float, vat_rate_code: str = 'STANDARD') -> Dict[str, Decimal]:
        ...
        """
        try:
            net = Decimal(str(net_amount)).quantize(
                Decimal('0.01'), rounding=ROUND_HALF_UP)
            vat_rate = self.VAT_RATES.get(
                vat_rate_code, self.VAT_RATES['STANDARD'])

            vat_amount = (
                net * vat_rate.rate).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    ...
    def generate_vat_return_data(self, period_start: str, period_end: str) -> Dict:
        ...
        try:
            ...
            total_value_sales_ex_vat = sum(
                t['net_amount'] for t in transactions)
            total_value_sales_inc_vat = sum(
                t['gross_amount'] for t in transactions)
    ...
    def submit_vat_return(self, vat_return_data: Dict) -> Dict:
        ...
        try:
            ...
            response = requests.post(
                url, json=vat_return_data, headers=headers)
