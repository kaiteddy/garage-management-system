...
def migrate_database():
    ...
    try:
        ...
        old_db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'garage.db')
        conn = sqlite3.connect(old_db_path)
        cursor = conn.cursor()

        # Get all existing vehicles
        cursor.execute('SELECT id, registration, make, model, color, fuel_type, mot_due, mileage, customer_id FROM vehicles')
        ...
        for vehicle in vehicles:
            print(f"Processing vehicle: {vehicle[1]}")  # vehicle[1] is registration
            ...
            if not new_vehicle.make:
                ...
                if vehicle[6]:  # mot_due
                    try:
                        new_vehicle.mot_expiry = datetime.strptime(vehicle[6], '%Y-%m-%d').date()
                    except ValueError:
                        pass
