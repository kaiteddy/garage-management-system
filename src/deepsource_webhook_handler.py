...
class DeepSourceWebhookHandler:
    ...
    def __init__(self, fix_manager: DeepSourceFixManager):
        ...
        if not self.webhook_secret:
            self.logger.warning("DEEPSOURCE_WEBHOOK_SECRET not set - webhook verification disabled")
    ...
    def handle_analysis_complete(self, payload: Dict) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Analysis complete for {repository.get('name', 'unknown')}")
            ...
            # Generate summary
            summary = self._generate_analysis_summary(processed_issues, analysis_data)
    ...
    def _process_issue(self, issue: Dict, repository: Dict) -> Dict:
        ...
        line_number = issue.get('location', {}).get('position', {}).get('begin', {}).get('line')
        description = issue.get('title', 'Unknown issue')
        
        # Generate fix ID
        fix_id = self.fix_manager.generate_fix_id(file_path, line_number, issue_type)
    ...
    def _generate_analysis_summary(self, processed_issues: List[Dict], 
        ...
        auto_fixable = len([i for i in processed_issues if i.get('auto_fixable', False)])
        already_fixed = len([i for i in processed_issues if i.get('status') == 'already_fixed'])
