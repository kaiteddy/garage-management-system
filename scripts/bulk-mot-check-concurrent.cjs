// Bulk MOT Check with Concurrent Processing
...
const CONFIG = {
  batchSize: 100,
  concurrentRequests: 5,  // Process 5 vehicles concurrently
  requestDelay: 200,      // 200ms between individual requests
...
async function processVehicle(vehicle) {
  try {
    // Clean registration from quotes if present
    const cleanedRegistration = vehicle.registration.replace(/\"/g, '');
    log(`Processing ${cleanedRegistration} (${vehicle.make} ${vehicle.model})`);
    const result = await checkMOTStatus(cleanedRegistration);
    ...
    if (result.status === 'SUCCESS' && result.data) {
      ...
      if (motTests.length > 0) {
        log(`Found ${motTests.length} MOT tests for ${cleanedRegistration}`);
        if (latestTest.expiryDate) {
          log(`Expiry date found: ${latestTest.expiryDate}`);
        } else {
          log(`No expiry date found in MOT data for ${cleanedRegistration}`);
        }
      }
      ...
      const updated = await updateVehicleMOT(cleanedRegistration, motData);
      if (updated) {
        log(`Updated MOT data for ${cleanedRegistration}`);
        stats.success++;
        return 'SUCCESS';
      }
    } 
    else if (result.status === 'NOT_FOUND') {
      await markVehicleChecked(cleanedRegistration);
      stats.notFound++;
      return 'NOT_FOUND';
    }
 ...
 */
async function main() {
  log('🚗 Starting Bulk MOT Status Check 🚗\n');
