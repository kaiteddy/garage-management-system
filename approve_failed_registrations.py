...
def approve_failed_registrations():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Failed to get failed registrations: {response.status_code}")
            return
        ...
        sample_expiry_dates = {
            "LN64XFG": (datetime.now() + timedelta(days=-5)).strftime("%Y-%m-%d"),  # Expired
            "FG34HIJ": (datetime.now() + timedelta(days=3)).strftime("%Y-%m-%d"),   # Critical
            "KL56MNO": (datetime.now() + timedelta(days=15)).strftime("%Y-%m-%d"),  # Due Soon
            "PQ78RST": (datetime.now() + timedelta(days=45)).strftime("%Y-%m-%d"),  # Valid
            "UV90WXY": (datetime.now() + timedelta(days=120)).strftime("%Y-%m-%d"), # Valid
            "ZA12BCD": (datetime.now() + timedelta(days=-15)).strftime("%Y-%m-%d"), # Expired
            "EF34GHI": (datetime.now() + timedelta(days=6)).strftime("%Y-%m-%d"),   # Critical
            "JK56LMN": (datetime.now() + timedelta(days=25)).strftime("%Y-%m-%d"),  # Due Soon
        }
        ...
        for failed_reg in failed_registrations:
            ...
            vehicle_data = sample_vehicle_data.get(registration, {"make": "Unknown", "model": "Vehicle", "year": 2020})
            mot_expiry = sample_expiry_dates.get(registration, (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d"))
                ...
                if response.status_code == 200:
                    ...
                    if result.get("success"):
                        ...
                        if resolve_response.status_code == 200:
                            ...
                            print(f"⚠️ Added {registration} but couldn't mark as resolved")
                    else:
                        print(f"❌ Failed to add {registration}: {result.get('error', 'Unknown error')}")
                else:
                    print(f"❌ HTTP error for {registration}: {response.status_code}")
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"📋 Total vehicles in system: {data.get('total_count', 0)}")
            else:
