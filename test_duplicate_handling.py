...
def test_duplicate_handling():
    ...
    try:
        ...
        result1 = import_service.import_csv_file(csv_file, 'customers', {'update_duplicates': False})
        
        if result1['success']:
            print(f'   ✅ Success! Imported: {result1.get("imported", 0)}, Failed: {result1.get("failed", 0)}, Duplicates: {result1.get("duplicates", 0)}')
        else:
            print(f'   ❌ Failed: {result1.get("error", "Unknown error")}')
        
        # Second import - should handle duplicates
        print('\n📥 Second import (should handle duplicates):')
        result2 = import_service.import_csv_file(csv_file, 'customers', {'update_duplicates': True})
        
        if result2['success']:
            print(f'   ✅ Success! Imported: {result2.get("imported", 0)}, Failed: {result2.get("failed", 0)}, Duplicates: {result2.get("duplicates", 0)}')
        else:
        ...
        result3 = import_service.import_csv_file(csv_file, 'customers', {'update_duplicates': False})
        
        if result3['success']:
            print(f'   ✅ Success! Imported: {result3.get("imported", 0)}, Failed: {result3.get("failed", 0)}, Duplicates: {result3.get("duplicates", 0)}')
        else:
...
def check_customer_data():
    ...
    try:
        ...
        cursor.execute('SELECT account_number, name, email, mobile FROM customers ORDER BY id DESC LIMIT 10')
        customers = cursor.fetchall()
        
        print('\n📊 Current customers in database:')
        print('=' * 60)
        for customer in customers:
            print(f'Account: {customer[0]} | Name: {customer[1]} | Email: {customer[2]} | Mobile: {customer[3]}')
