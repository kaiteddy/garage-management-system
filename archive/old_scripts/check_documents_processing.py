...
def check_documents_processing():
    ...
    try:
        ...
        vehicle_link_rate = (linked_vehicles / total_vehicles * 100) if total_vehicles > 0 else 0
        print(f'   Vehicles linked to customers: {linked_vehicles}/{total_vehicles} ({vehicle_link_rate:.1f}%)')
        ...
        job_link_rate = (linked_jobs / total_jobs * 100) if total_jobs > 0 else 0
        print(f'   Jobs linked to customers: {linked_jobs}/{total_jobs} ({job_link_rate:.1f}%)')
        ...
        invoice_link_rate = (invoice_job_links / total_invoices * 100) if total_invoices > 0 else 0
        print(f'   Invoices linked to jobs: {invoice_job_links}/{total_invoices} ({invoice_link_rate:.1f}%)')
        ...
        for job in linked_jobs_sample:
            desc = job[1][:40] + "..." if job[1] and len(job[1]) > 40 else job[1] or "No description"
