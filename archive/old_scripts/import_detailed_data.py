...
# Database path
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'src', 'garage.db')
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if 'invoice_number' in row and pd.notna(row['invoice_number']):
                cursor.execute("SELECT id FROM invoices WHERE invoice_number = ?", (str(row['invoice_number']),))
                result = cursor.fetchone()
                if result:
                    invoice_id = result['id']
            
            if 'job_number' in row and pd.notna(row['job_number']):
                cursor.execute("SELECT id FROM jobs WHERE job_number = ?", (str(row['job_number']),))
                result = cursor.fetchone()
                if result:
                    job_id = result['id']
            ...
            ''', (
                ...
                row.get('part_number', ''),
                row.get('description', ''),
                float(row.get('quantity', 0)) if pd.notna(row.get('quantity')) else 0,
                float(row.get('unit_price', 0)) if pd.notna(row.get('unit_price')) else 0,
                float(row.get('total_price', 0)) if pd.notna(row.get('total_price')) else 0,
                row.get('supplier', ''),
        ...
        conn.commit()
        conn.close()
        print(f"Successfully imported {len(df)} parts records from {csv_file_path}")
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            ''', (
                ...
                job_id,
                row.get('description', ''),
                float(row.get('hours', 0)) if pd.notna(row.get('hours')) else 0,
        ...
        conn.commit()
        conn.close()
        print(f"Successfully imported {len(df)} labour records from {csv_file_path}")
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if 'vehicle_registration' in row and pd.notna(row['vehicle_registration']):
                cursor.execute("SELECT id FROM vehicles WHERE registration = ?", (str(row['vehicle_registration']),))
                result = cursor.fetchone()
                if result:
        ...
        conn.commit()
        conn.close()
        print(f"Successfully imported {len(df)} advisory records from {csv_file_path}")
