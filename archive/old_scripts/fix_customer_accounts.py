...
def fix_customer_account_numbers():
    ...
    try:
        ...
        print(f'Found {len(malformed_accounts)} customers with malformed account numbers')
            ...
            if cleaned_account and cleaned_account != original_account:
                ...
                if not existing:
                    ...
                    cursor.execute('UPDATE customers SET account_number = ? WHERE id = ?', 
                                 (cleaned_account, customer_id))
                    print(f'   Fixed: "{original_account}" -> "{cleaned_account}" ({name})')
                    fixed_count += 1
                else:
                    print(f'   Skipped: "{original_account}" -> "{cleaned_account}" (duplicate exists)')
