        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('success'):
                    print(
                        f"   ✅ Success: {result.get('message', 'Vehicle added')}")
                    if 'vehicle_data' in result:
                        vehicle_data = result['vehicle_data']
                        print(
                            f"   📋 Make/Model: {vehicle_data.get('make', 'N/A')} {vehicle_data.get('model', 'N/A')}")
                        print(
                            f"   📅 MOT Expiry: {vehicle_data.get('mot_expiry_date', 'N/A')}")
                        print(
                            f"   🔍 Test Result: {vehicle_data.get('mot_test_result', 'N/A')}")
                else:
                    print(
