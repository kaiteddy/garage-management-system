...
def verify_database_connections():
    ...
    try:
        ...
        cursor.execute(
            'SELECT COUNT(*) FROM customers WHERE account_number IS NULL OR account_number = ""')
        customers_without_accounts = cursor.fetchone()[0]
        print(
            f'   Customers without account numbers: {customers_without_accounts}')

        # Sample account numbers
        cursor.execute(
            'SELECT account_number, name FROM customers WHERE account_number IS NOT NULL AND account_number != "" LIMIT 10')
        ...
        for link in sample_job_links:
            ...
            job_desc = link[1][:30] + "..." if link[1] and len(
                link[1]) > 30 else link[1] or "No description"
        ...
        for link in sample_invoice_links:
            ...
            print(
                f'     Invoice {link[0]} (£{link[1]}) -> {job_info} -> {customer_info}')
        ...
        try:
            ...
            print(
                f'   Payments linked to customers: {customer_linked_payments}')

            cursor.execute(
                'SELECT COUNT(*) FROM payments WHERE invoice_id IS NOT NULL')
        ...
        print(
            f'   Orphaned vehicles (invalid customer_id): {orphaned_vehicles}')
        ...
        if duplicate_accounts:
            print(
                f'   ⚠️  Duplicate account numbers found: {len(duplicate_accounts)}')
...
def check_eli_motors_linking():
    ...
    try:
        ...
        print(
            f'   Vehicles linked to ELI MOTORS customers: {eli_customer_vehicles}')
