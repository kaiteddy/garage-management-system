    ...
    try:
        ...
        for field_name, field_definition in new_fields:
            if field_name not in columns:
                try:
                    cursor.execute(
                        f'ALTER TABLE jobs ADD COLUMN {field_name} {field_definition}')
                    print(f"✅ Added field: {field_name}")
                except sqlite3.Error as e:
                    print(
                        f"⚠️  Warning: Could not add field {field_name}: {e}")
        ...
        for old_status, new_status in status_mapping.items():
            cursor.execute(
                'UPDATE jobs SET status = ? WHERE status = ?', (new_status, old_status))
        ...
        cursor.execute(
            'UPDATE jobs SET priority = "NORMAL" WHERE priority IS NULL OR priority = ""')
