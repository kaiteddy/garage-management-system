...
def process_uploaded_file(file):
    ...
    try:
        ...
        if registration_column is None:
            ...
            print(f"No matching registration column found, using first column: {registration_column}")
        ...
        print(f"Using columns - Registration: '{registration_column}', Mobile: '{mobile_column}', Name: '{name_column}'")
        ...
        for i, row in df.iterrows():
            ...
            if name_column and not pd.isna(row[name_column]):
                ...
                print(f"  -> Checking mobile extraction: mobile={mobile}, customer_name={customer_name}")
                if not mobile and customer_name:
                    import re
                    print(f"  -> Attempting to extract mobile from: {customer_name}")
                    ...
                    if mobile_match:
                        ...
                        print(f"  -> Extracted mobile from customer field: {mobile}")
                        ...
                        if 't:' in customer_name:
                            customer_name = customer_name.split('t:')[0].strip()
                            print(f"  -> Cleaned customer name: {customer_name}")
                        elif 'm:' in customer_name:
                            customer_name = customer_name.split('m:')[0].strip()
                            print(f"  -> Cleaned customer name: {customer_name}")
                    else:
                        print(f"  -> No mobile number found in customer field")
                else:
                    print(f"  -> Skipping mobile extraction: mobile={mobile}, customer_name={customer_name}")
            ...
            print(f"Row {i}: {reg_clean} | Mobile: {mobile} | Customer: {customer_name}")
        ...
        print(f"Final vehicle data: {len(vehicle_data)} vehicles with contact info")
    ...
    try:
        ...
        if integrated_mot_service:
            try:
                ...
                db_vehicles = integrated_mot_service.get_all_mot_vehicles(include_flagged=True, include_archived=False)
...
def upload_file():
    ...
    try:
        ...
        print(f"Processing result - vehicle_data: {len(vehicle_data_list) if vehicle_data_list else 0} vehicles, error: {error}")
        ...
        message_parts = []
        if results['success']:
            message_parts.append(f"Successfully added {len(results['success'])} vehicles")
        if results['failed']:
            message_parts.append(f"{len(results['failed'])} vehicles could not be found")
        if results['duplicates']:
            message_parts.append(f"{len(results['duplicates'])} vehicles were already in the system")

        final_message = '. '.join(message_parts) if message_parts else "No vehicles processed"
...
def send_single_sms(registration):
    ...
    try:
        ...
        if result['success']:
            flash(f'SMS sent successfully to {vehicle["mobile_number"]}', 'success')
        else:
...
def send_bulk_sms():
    ...
    try:
        ...
        for reg in selected_registrations:
            vehicle = next((v for v in vehicles if v['registration'] == reg), None)
...
def mark_vehicle_complete(registration):
    ...
    try:
        ...
        print(f"Vehicle {registration} marked as {'completed' if completed else 'not completed'}")
...
def update_failed_registration(failed_id):
    ...
    try:
        ...
        print(f"Updating failed registration {failed_id} with corrected registration: {corrected_registration}")
