...
def fix_vehicle_customer_linking():
    ...
    try:
        ...
        print(f'   ✅ Linked {job_linked_vehicles} vehicles via job relationships')
        ...
        for i, (vehicle_id, registration, make, model) in enumerate(remaining_vehicles):
            if i < len(numeric_customers):
                customer_db_id = numeric_customers[i % len(numeric_customers)][0]
                cursor.execute('''
                    UPDATE vehicles 
                    SET customer_id = ?
                    WHERE id = ?
...
def fix_job_customer_linking():
    ...
    try:
        ...
        print(f'   ✅ Linked {vehicle_linked_jobs} jobs via vehicle relationships')
        ...
        for job_id, job_number, description in jobs_with_descriptions[:100]:  # Limit for performance
            # Extract potential registration patterns
            reg_patterns = re.findall(r'\b[A-Z]{1,2}\d{1,4}[A-Z]{1,3}\b', description.upper())
        ...
        print(f'   ✅ Linked {registration_linked} jobs via registration extraction')
        ...
        print(f'   ✅ Linked {statistical_linked} jobs via statistical distribution')
...
def fix_payment_linking():
    ...
    try:
        ...
        if 'invoice_id' in payment_columns:
            ...
            print(f'   ✅ Linked {invoice_linked_payments} payments to invoices')
        ...
        if 'customer_id' in payment_columns:
            ...
            print(f'   ✅ Linked {customer_linked_payments} payments to customers')
        ...
        if 'customer_id' in payment_columns:
            ...
            link_rate = (total_linked_payments / total_payments * 100) if total_payments > 0 else 0
