...
def test_dvsa_connection():
    ...
    try:
        ...
        response = requests.get(
            "http://localhost:8002/api/dvsa/service-status", timeout=10)

        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                service_status = data.get('service_status', {})
                dvsa_status = service_status.get(
                    'dvsa_mot_api', {}).get('status', 'unknown')
...
def test_real_registration():
    ...
    try:
        response = requests.get(
            "http://localhost:8002/api/dvsa/vehicle/LN64XFG/mot", timeout=30)
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                if 'motTests' in mot_data:
                    print(
                        f"📊 Found {len(mot_data['motTests'])} MOT test records")
                return True
            else:
                print(
