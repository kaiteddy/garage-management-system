...
export function useVehicleData() {
  ...
  const fetchVehicleData = useCallback(async () => {
    try {
      ...
      console.log(`[useVehicleData] Raw vehicles array length: ${data.vehicles?.length || 0}`)
      ...
      if (!data.vehicles || !Array.isArray(data.vehicles)) {
        console.error("[useVehicleData] Invalid vehicles data structure:", data.vehicles)
        throw new Error("Invalid vehicles data structure received")
      }
      ...
      const enrichedVehicles: EnrichedVehicle[] =
        data.vehicles.map((vehicle: any, index: number) => {
          if (index < 3) {
            console.log(`[useVehicleData] Sample vehicle ${index}:`, vehicle)
          }
