  ...
  const loadConversationMessages = async (conversation: any) => {
    try {
      ...
      const response = await fetch(
        `/api/whatsapp/conversation/${conversation.customer_id}?phone=${encodeURIComponent(conversation.phone_number)}&limit=100`
      )
      const result = await response.json()
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Conversation with {selectedConversation.first_name} {selectedConversation.last_name}
                    </CardTitle>
                    <CardDescription>
                      Phone: {selectedConversation.phone_number} • {conversationMessages.length} messages
                    </CardDescription>
                  </div>
                  <Button
                    variant="outline"
                      ...
                      >
                        ...
                        >
                          <div className="text-sm font-medium mb-1">
                            {message.direction === 'outbound' ? 'ELI MOTORS' : selectedConversation.first_name}
                          </div>
                          ...
                          <div className="text-xs mt-2 opacity-75">
                            ...
                            {message.direction === 'outbound' && (
                              <span className="ml-2">
                                • {message.status} • £{(typeof message.cost === 'number' ? message.cost : parseFloat(message.cost || '0')).toFixed(4)}
                              </span>
                            )}
                          </div>
                        </div>
