...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const serviceHistory = await sql`
      ...
      ORDER BY cd.document_date DESC NULLS LAST, cd.document_number DESC
    `
    ...
    const lineItems = await sql`
      SELECT
        ...
        li.tax_rate
      FROM line_items li
      INNER JOIN customer_documents cd ON li.document_id = cd.id
      WHERE cd.customer_id = ${customerId}
    ...
    const receipts = await sql`
      ...
      FROM receipts r
      INNER JOIN customer_documents cd ON r.document_id = cd.id
      WHERE cd.customer_id = ${customerId}
