...
export async function GET() {
  try {
    ...
    // Get recent conversations (extended to 30 days to show more conversations)
    const recentConversations = await sql`
      SELECT
        c.*,
        COALESCE(cust.first_name,
          CASE
            WHEN c.customer_id LIKE 'test-%' THEN 'Test'
            ELSE 'Unknown'
          END
        ) as first_name,
        COALESCE(cust.last_name,
          CASE
            WHEN c.customer_id LIKE 'test-%' THEN 'Customer'
            ELSE 'Customer'
          END
        ) as last_name,
        COALESCE(cust.email, 'No email') as email,
      ...
      GROUP BY c.id, c.customer_id, cust.first_name, cust.last_name, cust.email
