...
export async function GET() {
  try {
    console.log('[API-V2-DATA] 🔄 Loading vehicles data...')

    /* 1️⃣  core CSV data (Customers / Vehicles / …) */
    const core = await getConnectedData()
    console.log(`[API-V2-DATA] 📋 Core vehicles found: ${core.vehicles.size}`)

    /* 2️⃣  anything the user uploaded in this session */
    const uploaded = getUploadedData()
    console.log(`[API-V2-DATA] 📤 Uploaded vehicles found: ${uploaded.vehicles.length}`)
    ...
    const allVehicles = [...coreVehicles, ...uploadedVehicles]
    const allCustomers = [...coreCustomers, ...uploadedCustomers]

    console.log(`[API-V2-DATA] ✅ Vehicles data loaded: ${allVehicles.length} total vehicles`)
    console.log(`[API-V2-DATA] ✅ Customers data loaded: ${allCustomers.length} total customers`)
