...
// DELETE endpoint to clear test verification codes
export async function DELETE() {
  try {
    console.log("[VERIFICATION-CODES] 🗑️ Clearing test verification codes...")

    // Delete all verification codes (keeping the table structure)
    const result = await sql`
      DELETE FROM verification_codes
      WHERE TRUE
    `

    console.log(`[VERIFICATION-CODES] ✅ Cleared ${result.count || 0} verification codes`)
    ...
    console.error("[VERIFICATION-CODES] ❌ Error clearing codes:", error)
    return NextResponse.json({
      success: false,
