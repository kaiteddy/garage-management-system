...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { registration, updates } = body
    ...
    console.log(`🔄 [API] Updating vehicle data for registration: ${registration}`)
    console.log(`📝 [API] Updates:`, updates)

    // Update vehicle data
    const vehicle = await VehicleService.updateVehicleData(registration, updates)

    return NextResponse.json({
      success: !!vehicle,
      data: vehicle,
      message: vehicle ? "Vehicle data updated successfully" : "Vehicle not found",
    })
  } catch (error) {
