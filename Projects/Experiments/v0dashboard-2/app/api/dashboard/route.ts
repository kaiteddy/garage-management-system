...
export async function GET() {
  try {
    ...
    const [
      ...
      // Active Job Sheets count (only OPEN job sheets)
      sql`
        SELECT COUNT(*) as count
        FROM customer_documents
        WHERE document_type = 'JS'
        AND (status = '0' OR status IS NULL OR status = '3')
      `,
    ...
    const activeJobSheets = parseInt(activeJobSheetsCount[0].count)
    ...
    console.log(`[DASHBOARD-API] Stats: ${customers} customers, ${vehicles} vehicles, ${activeJobSheets} active job sheets, ${criticalMots} critical MOTs`)
