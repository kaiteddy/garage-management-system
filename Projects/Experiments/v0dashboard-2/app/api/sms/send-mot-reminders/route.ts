  ...
  try {
    ...
    for (const [customerId, customerData] of customerVehicles) {
      ...
      if (!dryRun && TwilioService.isConfigured()) {
        ...
        if (channel === 'whatsapp' && vehicles.length === 1) {
          // Use WhatsApp template for single vehicle reminders
          const vehicle = vehicles[0]
          const motExpiryDate = new Date(vehicle.mot_expiry_date).toLocaleDateString('en-GB', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
          })
          const daysRemaining = Math.abs(vehicle.days_until_expiry || 0).toString()
          ...
          // Use regular SMS/WhatsApp for multi-vehicle or SMS channel
          smsMessage.channel = channel as 'sms' | 'whatsapp'
          result = await TwilioService.sendMessage(smsMessage)
        }
        results.push({
