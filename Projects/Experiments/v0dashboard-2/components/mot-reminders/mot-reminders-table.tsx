...
export function MOTRemindersTable({ vehicles, isLoading }: MOTRemindersTableProps) {
  ...
  const getStatusBadge = (status: EnrichedVehicle["motStatus"]) => {
    ...
    const label = status.charAt(0).toUpperCase() + status.slice(1).replace("-", " ").toUpperCase()

    return (
      <Badge variant={config.variant} className={`${config.className} uppercase`}>
        {label}
      </Badge>
    )
  }
            ...
            {paginatedVehicles.map((vehicle) => {
              ...
              return (
                <TableRow key={vehicle.id}>
                  <TableCell className="text-sm font-medium">{formatDisplayDate(vehicle.workDue)}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className="text-xs uppercase">
                      {vehicle.archived ? "ARCHIVED" : "ACTIVE"}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-registration uppercase">{vehicle.registration?.toUpperCase()}</TableCell>
                  <TableCell className="font-medium uppercase">{vehicle.make?.toUpperCase()}</TableCell>
                  <TableCell className="uppercase">
                    {vehicle.model?.toUpperCase()}
                    {vehicle.year && <span className="text-muted-foreground ml-1">({vehicle.year})</span>}
                  </TableCell>
                  <TableCell className="font-medium uppercase">{customerName?.toUpperCase()}</TableCell>
                  <TableCell>
