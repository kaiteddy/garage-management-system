...
export function PartsTable({ parts: propsParts, searchTerm, setSearchTerm, activeTab, setActiveTab }: PartsTableProps = {}) {
  ...
  React.useEffect(() => {
    if (propsParts) {
      // Convert provided parts data to StockItem format
      const convertedParts = propsParts.map((part: any) => ({
        id: part.id,
        partNumber: part.partNumber,
        description: part.description,
        category: part.category,
        supplier: part.supplier,
        quantityInStock: part.stockLevel || part.quantityInStock || 0,
        priceRetailNet: part.sellPrice || part.priceRetailNet || 0,
        costNet: part.costPrice || part.costNet || 0,
        location: part.location || '',
        minimumStockLevel: part.minStock || part.minimumStockLevel || 0
      }))
      setData(convertedParts as StockItem[])
      setLoading(false)
    } else {
      // Fetch from API
      ...
      <div className="flex items-center py-4">
        <div className="relative w-full max-w-sm">
          ...
          <Input
            ...
            value={searchTerm ?? (table.getColumn("description")?.getFilterValue() as string) ?? ""}
            onChange={(event) => {
