...
export function MOTCheck() {
  // Helper function to determine actual MOT status
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {result.motStatus && (
                    <Badge variant={getActualMotStatus(result.motStatus, result.motExpiryDate) === 'Expired' ? 'destructive' : 'default'} className="gap-1">
                      {getActualMotStatus(result.motStatus, result.motExpiryDate) === 'Expired' ? (
                        <AlertTriangle className="h-3.5 w-3.5" />
                      ) : (
                        <CheckCircle className="h-3.5 w-3.5" />
                      )}
                      {getActualMotStatus(result.motStatus, result.motExpiryDate)}
                    </Badge>
                  )}
                    ...
                    {result.motExpiryDate && (
                      <div className="space-y-2">
                        <div className="text-sm text-muted-foreground">
                          {getActualMotStatus(result.motStatus, result.motExpiryDate) !== 'Expired' ? 'Expiry Date' : 'Last Test Date'}
                        </div>
                        <div className="font-medium">
