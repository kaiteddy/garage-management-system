...
// Use direct Neon URL for neon client (not Prisma Accelerate URL)
const dbUrl = process.env.NEON_DATABASE_URL || process.env.DIRECT_URL || process.env.DATABASE_URL
if (!dbUrl) {
  console.error("[NEON-CLIENT] ERROR: No database URL found in environment variables")
} else {
...
try {
  if (!dbUrl) {
    throw new Error("No database URL found. Please set NEON_DATABASE_URL, DIRECT_URL, or DATABASE_URL")
  }

  // Check if URL is a Prisma Accelerate URL (not compatible with neon client)
  if (dbUrl.startsWith('prisma+postgres://')) {
    throw new Error(`Invalid database URL format for neon client. Expected postgresql:// but got prisma+postgres://. Please use NEON_DATABASE_URL or DIRECT_URL instead.`)
  }
