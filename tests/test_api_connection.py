...
def test_vehicle_lookup():
    ...
    try:
        ...
        if vehicle_info:
            ...
            print(f"✓ Vehicle: {vehicle_info.get('make', 'Unknown')} {vehicle_info.get('model', 'Unknown')}")
            print(f"✓ MOT expiry: {vehicle_info.get('mot_expiry_date', 'Unknown')}")
            print(f"✓ Days until expiry: {vehicle_info.get('days_until_expiry', 'Unknown')}")
            return True
        else:
            print(f"⚠ No data found for {test_registration} (this may be expected for test registration)")
