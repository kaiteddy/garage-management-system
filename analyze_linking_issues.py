...
def analyze_customer_id_patterns():
    ...
    try:
        ...
        print(f'   Numeric only (e.g., "004", "010"): {len(numeric_only)} customers')
        print(f'   Alpha-numeric (e.g., "AAN001", "GEO001"): {len(alpha_numeric)} customers')
        print(f'   Decimal format (e.g., "0.0", "1.0"): {len(decimal_format)} customers')
...
def create_customer_id_mapping():
    ...
    try:
        ...
        for acc, num_id, alpha_id, name in mappings:
            print(f'     Account: {acc} | Numeric: {num_id} | Alpha: {alpha_id} | Name: {name}')
