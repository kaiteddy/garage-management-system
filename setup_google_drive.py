    ...
    if credentials_path.exists():
        ...
        try:
            ...
            if 'installed' in creds:
                ...
                missing_fields = [field for field in required_fields if field not in creds['installed']]
                ...
                print("❌ Credentials file format is invalid (missing 'installed' section)")
                return False
...
def test_google_drive_service():
    ...
    try:
        ...
        if service.is_available():
            ...
            if status['connected']:
                print(f"✅ Connected to Google Drive as: {status.get('user_email', 'Unknown')}")
            else:
                print(f"⚠️  Not connected to Google Drive: {status.get('error', 'Unknown error')}")
                print("   This is normal for first-time setup - you'll need to authenticate via the web interface")
